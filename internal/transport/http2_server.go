	...
	for {
		...
		case <-idleTimer.C:
			...
			if val <= 0 {
				...
				t.Drain([]byte("max_idle"))
				return
			}
			idleTimer.Reset(val)
		case <-ageTimer.C:
			t.Drain([]byte("max_age"))
...
func (t *http2Server) Drain(debugData []byte) {
	t.mu.Lock()
	defer t.mu.Unlock()
	if t.drainEvent != nil {
		return
	...
	t.controlBuf.put(&goAway{code: http2.ErrCodeNo, debugData: debugData, headsUp: true})
}
